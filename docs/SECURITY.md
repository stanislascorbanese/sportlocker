...
| 19 | ~~Résorber les 43 advisories ≥ high accumulées depuis juin (dont 3 critical)~~ | ✅ **done 2026-09-10** — la CI bloquait toutes les PR sur `audit-node` sans qu'une ligne de code ait changé : un audit rougit seul, au rythme des publications. Traitement : `next` 15.5.18 → **15.5.25** (règle les deux RCE non authentifiées, `GHSA-p293-qw3h-jr36` sur hôte Windows et `GHSA-2xp9-vwfh-vxw4` sur l'API d'optimisation d'images) ; treize `pnpm.overrides` ajoutés pour le transitif (`brace-expansion`, `fast-uri`, `js-yaml`, `nanoid`, `postcss`, `svgo`, `sharp`, `browserslist`, `ip-address`, `smol-toml`, `find-my-way`, `@fastify/static`). `brace-expansion` et `fast-uri` cohabitent en plusieurs majeures dans l'arbre, d'où la forme `paquet@majeure` : un override unique aurait imposé la même version à des paquets écrits pour l'ancienne API. Seul `astro` reste tracé (cf. #18), faute de correctif en 5.x. | — | — |
