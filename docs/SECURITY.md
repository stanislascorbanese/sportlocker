...
| 13 | ~~`pnpm.overrides` pour patcher fast-jwt (3 critical), fast-uri (high), @xmldom/xmldom (high) → passer `audit-node` en hard~~ | ✅ **done** — voir `package.json#pnpm.overrides`. Passe de 58 vulns (3 critical, 22 high) à 25 vulns (0 critical, 2 high). Toutes les GHSAs ≥ high sont désormais résolues (overrides + upgrades `tmp`/`vitest`/`astro`/`fastify`) — **`IGNORED_GHSAS` est vide**, 0 vuln ≥ high dans tout le monorepo. `audit-node` retiré du mode soft. | — |
