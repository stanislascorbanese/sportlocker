...
| 13 | ~~`pnpm.overrides` pour patcher fast-jwt (3 critical), fast-uri (high), @xmldom/xmldom (high) → passer `audit-node` en hard~~ | ✅ **done** — voir `package.json#pnpm.overrides`. Passe de 58 vulns (3 critical, 22 high) à 25 vulns (0 critical, 2 high). GHSAs restantes (transitives/major bumps + `tmp` couvert par override) ignorées en CI via filtre `jq` (items #14–#16). `audit-node` retiré du mode soft. | — |
...
| 16 | **Migration `vitest` 1.6 → 4.1+ pour résoudre `GHSA-5xrq-8626-4rwp` (lecture/exécution de fichier arbitraire via le serveur Vitest UI, critical)** — devDep uniquement (jamais en prod) et exploitable seulement quand `vitest --ui` écoute, ce qu'on ne lance ni en CI (`vitest run`) ni sur les serveurs. Saut de 3 majors sur `apps/citizen`, `apps/dashboard`, `services/api` + `@vitest/coverage-v8` → chantier dédié. GHSA ignorée en CI via `IGNORED_GHSAS`. | T3 2026 | 2 j |
