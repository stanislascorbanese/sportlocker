...
| 18 | **Migration `astro` 5 → 6 (+ CI Node ≥ 22.12) pour résoudre `GHSA-8hv8-536x-4wqp` (XSS reflected via slot non échappé) et `GHSA-2pvr-wf23-7pc7` (SSRF Host header sur error page prerender), high.** **Tracées dans `IGNORED_GHSAS` depuis le 2026-06-16.** Patch disponible UNIQUEMENT en Astro 6.x (≥6.3.3 / ≥6.4.6) ; on est resté en Astro 5 (cf. #14) car Astro 6 exige Node ≥ 22.12 alors que la CI tourne en Node 20 — bumper Node en CI est un prérequis. Risque réel faible : `apps/web` est `output: static` (HTML prérendu, pas de SSR ni de slot dynamique piloté par la requête), donc XSS reflected et SSRF runtime non exploitables en prod. À retirer d'`IGNORED_GHSAS` une fois la migration Astro 6 + Node 22 faite. **Blocage identifié 2026-06-22 (tentative reportée) :** le vrai prérequis n'est pas seulement Node 22 — `@astrojs/react` n'a PAS de release stable pour Astro 6 (uniquement `6.0.0-alpha/beta` ; stable = 5.0.7, peer Astro ≤5), or toute la vitrine repose sur des îlots React → migrer mettrait une intégration React **beta en production**. De plus `@astrojs/tailwind` (peer `astro ^3‖^4‖^5`, Tailwind 3 only) ne supporte pas Astro 6 → il faudra recâbler **Tailwind 3 via PostCSS** (éviter une cascade Tailwind 3→4). **Reprendre quand `@astrojs/react` v6 stable est publié**, pas avant : l'écosystème n'est pas mûr et le risque de casser la vitrine dépasse celui des 2 advisories (faible, site statique). L'item #17 (esbuild) sera résolu par la même montée Astro/Vite. **Maj 2026-09-10 :** une troisième advisory astro s'ajoute, `GHSA-26w7-cxv4-gfx2` (RCE via l'optimisation d'images AVIF), **critical** cette fois. Même blocage, même raisonnement d'exposition : en `output: static` l'optimisation d'images tourne au build sur des fichiers du dépôt, jamais à la requête sur un fichier fourni par un visiteur — le vecteur de l'advisory n'existe pas ici. Tracée dans `IGNORED_GHSAS`. La sévérité critique ne change pas la décision mais raccourcit le délai acceptable : si `@astrojs/react` v6 stable n'est pas publié d'ici la fin 2026, il faudra trancher autrement — désactiver `astro:assets` au profit d'images pré-optimisées à la main, par exemple. | T4 2026 | 2-3 j |
| 19 | **Résorber les 43 advisories ≥ high accumulées depuis juin (dont 3 critical)** | ⚠️ **partiel — 2026-09-10.** Fait : `next` 15.5.18 → **15.5.25**, qui règle les deux RCE non authentifiées (`GHSA-p293-qw3h-jr36`, hôte Windows, sans objet ici ; `GHSA-2xp9-vwfh-vxw4`, API d'optimisation d'images AVIF, bien réelle). Non fait : le transitif. Une première tentative a posé treize `pnpm.overrides` d'un coup et **a cassé la CI** — `js-yaml: >=4.3.2` supprime l'export par défaut que `@astrojs/markdown-remark` importe, donc `astro check` ne démarrait plus ; et `@fastify/static: >=10.1.1` serait parti en production alors que la v10 exige Fastify 5, sur une API en Fastify 4. Le lot a été retiré en entier. **Leçon, à appliquer la prochaine fois : un override par commit, chacun validé par `pnpm build` complet avant d'être poussé.** Les ~41 advisories restantes sont des DoS par entrée malveillante dans des utilitaires d'analyse (URL, motifs, YAML, CSS) dont aucun ne reçoit d'entrée contrôlée par un visiteur : js-yaml ne lit que le frontmatter du dépôt au build, fast-uri ne voit que des routes déclarées. `audit deps` n'étant pas une vérification requise par le ruleset, elles ne bloquent aucune PR. **Maj 2026-09-12 :** le transitif est traité, en appliquant la leçon ci-dessus — trois overrides seulement, ciblés, et la porte `audit-node` rejouée à l'identique avant de pousser. Deux overrides seulement, tous deux des montées de correctif à l'intérieur de la plage déclarée par les dépendants : `fast-uri` ≥4.1.1 (résout 6 advisories — confusion d'hôte et SSRF ; 4.1.0 → 4.1.4) et `postcss` ≥8.5.12 (résout 2 — traversée de chemin via `sourceMappingURL` ; 8.4.31 → 8.5.28). `@fastify/static` a été **retiré du lot en cours de route** : le blocage Fastify 4 a bien disparu (cf. item #15, l'API est en `fastify ^5.10.0`), mais le paquet est transitif et son seul dépendant, `@fastify/swagger-ui@5.2.6`, déclare `^9.1.2` — forcer 10.x le sortirait de sa plage testée, c'est-à-dire refaire exactement l'erreur de juin. Traité séparément en item #21. Restent donc **6** advisories, toutes tracées : trois `js-yaml` (item #20), deux `sharp` (item #18), une `@fastify/static` (item #21). L'audit passe de 14 non tracées à **0**. | T4 2026 | ✅ transitif fait |
| 20 | **Retirer l'épingle `js-yaml: 4.1.1` et lever `GHSA-52cp-r559-cp3m`, `GHSA-5p4m-2wfm-xmqj`, `GHSA-2883-xcg3-v3hh`** (DoS CPU quadratique sur les clés de fusion YAML, high). **Tracées dans `IGNORED_GHSAS` depuis le 2026-09-12.** L'épingle est délibérée et documentée dans `package.json#pnpm.overrides` : c'est **4.2.0** qui a retiré l'export par défaut du build ESM, et `@astrojs/markdown-remark` fait `import yaml from 'js-yaml'` — dès 4.2.0, `astro check` ne démarre plus. Vérifié en dépaquetant 4.1.1, 4.2.0, 4.3.0 et 4.3.2 ; une tentative de bump a déjà cassé la CI (cf. item #19). Exposition réelle nulle : ici `js-yaml` ne lit que le frontmatter du dépôt au build, jamais du YAML fourni par un visiteur. Le correctif réel est côté Astro — migration vers les imports nommés — donc même dépendance que l'item #18. À retirer d'`IGNORED_GHSAS` le jour où Astro n'importe plus l'export par défaut. | T4 2026 | dépend d'Astro |
| 21 | **Monter `@fastify/swagger-ui` 5.2.6 → 6.1.1 pour lever `GHSA-83w8-p2f5-377r`** (contournement de garde de route par traversée de chemin dans `@fastify/static`, high). **Tracée dans `IGNORED_GHSAS` depuis le 2026-09-12.** `@fastify/static` est transitif : seul `@fastify/swagger-ui` le tire, et la 5.2.6 déclare `^9.1.2` alors que le correctif est en 10.1.1. Un override vers 10.x sortirait swagger-ui de sa plage testée — l'erreur exacte de l'item #19. La 6.1.1 déclare elle-même `^10.1.0`, donc la montée résout l'advisory **sans** override. C'est un bump majeur d'une dépendance directe de l'API (`services/api/src/plugins/swagger.ts`, options de `register`), donc sa propre PR, validée par la suite de tests api complète et un démarrage réel de `/docs`. **Priorité haute — contrairement aux autres exceptions de la liste, celle-ci est réellement atteignable : `/docs` est publique et sans authentification.** | T4 2026 | 0.5 j |
| 22 | **`ecdsa` — `PYSEC-2026-1325`, sans correctif amont.** **Ignorée dans `pip-audit` depuis le 2026-09-13.** Le paquet est tiré transitivement par `python-jose[cryptography]`. Ce n'est pas une montée qu'on repousse : **aucune version corrigée n'existe**, il n'y a rien à installer. Et le chemin de code n'est jamais atteint — le firmware ne vérifie que des JWT **HS256** (`services/firmware/src/sportlocker_firmware/jwt_verify.py`, `JWT_ALGORITHM = "HS256"`), un HMAC : aucune signature à courbe elliptique n'est produite ni vérifiée sur la borne. À retirer si un correctif paraît, ou le jour où les QR d'ouverture passeraient à un algorithme asymétrique — ce qui serait de toute façon une décision de conception, pas un bump. `setuptools` (PYSEC-2026-3447), lui, est corrigé : il vient de l'image du runner et non de nos `requirements.txt`, donc il est monté dans le workflow plutôt qu'épinglé dans un fichier où il n'a rien à faire. | — | ✅ traité |
