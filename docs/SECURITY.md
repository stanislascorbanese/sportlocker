...
| 13 | ~~`pnpm.overrides` pour patcher fast-jwt (3 critical), fast-uri (high), @xmldom/xmldom (high) → passer `audit-node` en hard~~ | ✅ **done** — voir `package.json#pnpm.overrides`. Passe de 58 vulns (3 critical, 22 high) à 25 vulns (0 critical, 2 high). GHSAs restantes (major bumps `astro`/`fastify` + `tmp`/`vitest` résolus par override/upgrade) — 2 GHSAs encore ignorées en CI via filtre `jq` (items #14, #15). `audit-node` retiré du mode soft. | — |
