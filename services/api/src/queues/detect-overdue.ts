...
import { distributors, itemTypes, items, notificationLogs, reservations, users } from '../db/schema.js'
import { notifyUserPush } from '../lib/push-notify.js'

/**
 * Marque `overdue` toute réservation active dont `due_at` est dépassé, applique
 * une pénalité de `trust_score` au user, puis lui envoie un rappel push.
 *
 ...
 * seule fois** par l'UPDATE → exactement une pénalité + une tentative de
 * notification, pas besoin de dédoublonner via `notification_logs` (au
 * contraire de slot-reminders dont le SELECT peut re-matcher la même row).
 *
 * Pénalité : `OVERDUE_TRUST_PENALTY` points par réservation en retard (10 par
 * défaut, 0 = désactivé). Le score est clampé à 0 côté SQL via `GREATEST`,
 * pour respecter le CHECK `trust_score BETWEEN 0 AND 100`. L'invariant
 * "1 résa vivante max par user" (idx_reservations_one_live_per_user, migr.
 * 0008) garantit en pratique ≤ 1 overdue par user et par run ; l'agrégation
 * par user reste défensive (correcte si l'invariant venait à être relâché).
 *
 ...
 * et observabilité). Les compteurs de pénalités/notifications sont loggés,
 * pas retournés, pour préserver la signature consommée ailleurs.
 */
...
export async function runDetectOverdue(log: FastifyBaseLogger): Promise<number> {
  ...
  // Pénalité trust_score : OVERDUE_TRUST_PENALTY points par retard, cumulés
  // par user sur ce run, clampés à 0 côté SQL (CHECK trust_score >= 0). Un
  // UPDATE par user distinct (volumétrie cron faible). Désactivé si penalty=0.
  const penalty = env.OVERDUE_TRUST_PENALTY
  if (penalty > 0) {
    const penaltyByUser = new Map<string, number>()
    for (const row of overdue) {
      penaltyByUser.set(row.userId, (penaltyByUser.get(row.userId) ?? 0) + penalty)
    }
    for (const [userId, total] of penaltyByUser) {
      await db
        .update(users)
        .set({
          trustScore: sql`GREATEST(${users.trustScore} - ${total}, 0)`,
          updatedAt: now,
        })
        .where(eq(users.id, userId))
    }
    log.info({ users: penaltyByUser.size, penaltyPerOverdue: penalty }, 'overdue trust penalty applied')
  }
