...
const TONE_STYLE: Record<LockerCellTone, { label: string; cls: string; dot: string }> = {
  'idle-empty':  { label: 'Vide',     cls: 'border-dashed border-white/15 bg-navy-900',         dot: 'bg-white/30' },
  'idle-loaded': { label: 'Idle',     cls: 'border-emerald-500/30 bg-emerald-500/[0.06]',       dot: 'bg-emerald-400' },
  'reserved':    { label: 'Réservé',  cls: 'border-sky-500/30 bg-sky-500/[0.06]',               dot: 'bg-sky-400' },
  'active':      { label: 'En cours', cls: 'border-amber-500/30 bg-amber-500/[0.06]',           dot: 'bg-amber-400' },
  'returning':   { label: 'Retour',   cls: 'border-purple-500/30 bg-purple-500/[0.06]',         dot: 'bg-purple-400' },
  'fault':       { label: 'Panne',    cls: 'border-rose-500/30 bg-rose-500/[0.06]',             dot: 'bg-rose-400' },
}
...
export default async function DistributorDetailPage(
  ...
  try {
    ...
    fetchError = err instanceof Error ? err.message : 'API indisponible'
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="min-w-0">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-3 text-sm text-white/55">
            ...
            <span>
              ...
              {' '}
              <span className="text-white/40">/ {summary.total} vides</span>
            </span>
            <span className="text-white/40">·</span>
            <span>{summary.idleLoaded} chargés · {summary.active + summary.reserved} en circulation</span>
            {summary.fault > 0 && (
              <>
                <span className="text-white/40">·</span>
                <span className="text-rose-300">{summary.fault} en panne</span>
              </>
            )}
          </div>
        </div>
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card label="Casiers libres pour chargement" value={`${summary.loadable}`} accent="emerald" />
        <Card label="Articles chargés (idle)"        value={`${summary.idleLoaded}`} />
        <Card label="Réservations en cours"          value={`${summary.reserved + summary.active}`} />
        <Card label="Batterie / dernier signe">
          <div className="flex items-center gap-3">
            <BatteryGauge percent={data.batteryPercent} />
            <span className="text-xs text-white/60">{fmtRelative(data.lastSeenAt)}</span>
          </div>
        </Card>
      </section>
      ...
      <section className="rounded-xl border border-white/10 bg-navy-800 p-4 sm:p-6">
        <div className="mb-4 flex flex-wrap items-baseline justify-between gap-3">
          <h3 className="font-display text-lg">Grille des casiers</h3>
          <Legend />
        </div>
        <LockerGrid lockers={data.lockers} />
      </section>
...
function Legend() {
  const entries: Array<[LockerCellTone, string]> = [
    ['idle-empty', 'Vide'],
    ['idle-loaded', 'Idle'],
    ['reserved', 'Réservé'],
    ['active', 'En cours'],
    ...
    <ul className="flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-white/55">
      {entries.map(([tone, label]) => (
        <li key={tone} className="inline-flex items-center gap-1.5">
          <span className={cn('h-1.5 w-1.5 rounded-full', TONE_STYLE[tone].dot)} />
          {label}
        </li>
      ))}
    </ul>
...
function LockerGrid({ lockers }: { lockers: DistributorLocker[] }) {
  if (lockers.length === 0) {
    return (
      <p className="text-sm text-white/40">Aucun casier configuré pour ce distributeur.</p>
    )
  }
      ...
      {lockers.map((l) => {
        ...
        return (
          <li
            ...
            aria-label={`Casier ${l.position + 1} — ${tone.label}`}
          >
            ...
            {l.itemType ? (
              ...
              <p className="min-h-[2.5rem] text-sm text-white/30">
                {cell.tone === 'fault' ? 'À diagnostiquer' : 'Casier vide'}
              </p>
            )}
          </li>
        )
