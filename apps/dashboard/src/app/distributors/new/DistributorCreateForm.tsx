...
export function DistributorCreateForm({
  ...
  function onAddressSelect(a: AddressAutofill) {
    ...
    if (match) {
      ...
      setAutofillNotice(
        t.formCommuneAutoSelected.replace('%s', match.name).replace('%i', a.citycode),
      )
    } else if (a.citycode) {
      setAutofillNotice(
        t.formCommuneAutoMissing.replace('%i', a.citycode).replace('%c', a.city),
      )
    } else {
    ...
    <form action={formAction} className="space-y-5">
      ...
      {communes.length > 0 ? (
        <label className="block">
          ...
          >
            <option value="" disabled>{t.formCommunePlaceholder}</option>
            {communes.map((c) => (
        ...
        <Field
          name="communeId"
          label={t.formCommuneUuid}
          placeholder="76e317a8-616c-486c-87cb-73253f411d8a"
          hint={t.formCommuneUuidHint}
      ...
      <Field
        name="lockerCount"
        label={t.formLockerCount}
        type="number"
        defaultValue="4"
        min={1}
        max={64}
        hint={t.formLockerCountHint}
      ...
      <div className="flex items-center justify-end gap-3 pt-2">
        ...
        <SubmitButton idle={t.btnCreate} pendingLabel={t.btnCreating} />
      </div>
    </form>
  )
}
